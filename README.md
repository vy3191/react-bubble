...
Ans: A token is used to determine the authorization. If the authenticated user wants to read or write certain features on the application, token is the key in deciding the authorization.

- [X] What steps can you take in your web apps to keep your data secure?
Ans: I will make sure that user is Authenticated first, and then verify if the user is Authorized or not by checking the token which is issued when the user logins in.
